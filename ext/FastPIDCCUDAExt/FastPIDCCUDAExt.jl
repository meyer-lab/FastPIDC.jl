...
function joint_counts_kernel_batched(data, counts, n, m, k_bins)
    ...
    if x > n || z > n; return nothing; end
    if x == z; return nothing; end
...
function mi_si_kernel_batched(counts, marginals, mi_matrix, si_matrix, n, m, k_bins)
    ...
    if x > n || z > n || x == z; return nothing; end
...
"""
    ...
    Grid: (n, n)
"""
function puc_accumulation_kernel_batched(si_matrix, mi_matrix, puc_scores, marginals, n, k_bins)
    x = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    z = (blockIdx().y - 1) * blockDim().y + threadIdx().y
...
function FastPIDC.compute_puc_full_cuda(nodes, config, base)
    ...
    if config.verbose
        println("[FastPIDC] GPU Fully Batched PUC: Processing $num_nodes x $num_nodes pairs...")
    end
